...
def assemble_data(
    ...
    print(f'Got topo layers: {type(topo)}')

    snow_class = get_snow_climatology(aoi=aoi_gdf, crs=crs, fp=fp_snowclimate, ref_grid=ref)
    print(f"Got snow climatology: {type(snow_class)}")

    
    # 5. Loop through date pairs
    # for idx, (start_date, end_date) in enumerate(date_pairs) :
    # 5a. Get AORC for each range 
    # aorc = get_aorc_layers(aoi=aoi_gdf, date_pairs=date_pairs, crs=crs, ref_grid=ref)
...
def get_aorc_layers(
    ...
    #             ) for yr in years]
    # # print(fileset)
    # ds_full = xr.open_mfdataset(fileset, 
    #                             preprocess=fix_aorc_dates,
    #                             engine='zarr')
